...
- [Miscellaneous](#miscellaneous)
    ...
    - [Integers to Roman Numbers](#11-integers-to-roman-numbers)
...
Solution<br>
- Store the values and the symbol Strings in two arrays matching the index
- Include combined values like IV, IX in these arrays
- given a number first find a such that number is greater than value. Do this starting from the largest value
- calculate the number of times the selected value is going to be applied count = number/selected value
- calculate the remainder from the number and repeat the process until number equals zero: number = number - selected value * count
[Solution](src/main/java/miscellaneous/NumbersToRoman.java)<br>
